...
class VentanaDimensiones:
    ...
    def mostrar_dimensiones(self, codigo_pensamiento):
        ...
        ttk.Label(frame_intensidad, text="Intensidad (0-10, opcional):").pack(side=tk.LEFT, padx=5)
    ...
    def cargar_dimensiones_existentes(self, codigo_pensamiento):
        try:
            conn = sqlite3.connect('../../data/data/db_psicologia_clinic.db')
            cursor = conn.cursor()
            ...
            dimensiones_hoy = cursor.fetchone()
            if dimensiones_hoy:
                ...
                self.dimensiones[codigo_pensamiento]['intensidad'].set(dimensiones_hoy[2] if dimensiones_hoy[2] else 0)
        ...
        try:
            ...
            if dimensiones:
                ...
                for dim in dimensiones:
                    duracion = dim[1] if dim[1] is not None else ''
                    intensidad = dim[2] if dim[2] is not None else ''
                    tree.insert('', 'end', values=(dim[0], duracion, intensidad))
    ...
    def guardar_dimension(self, codigo_pensamiento):
        ...
        try:
            ...
            """, (
                ...
                dims['intensidad'].get() if dims['intensidad'].get() > 0 else None
            ))
    ...
    def actualizar_dimensiones_fecha(self, event=None):
        """Actualizar las dimensiones cuando cambie la fecha"""
        if self.pensamiento_seleccionado:
            self.mostrar_dimensiones(self.pensamiento_seleccionado)
