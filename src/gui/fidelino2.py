...
class VentanaEstadisticas:
    ...
    def actualizar_graficos(self, event=None):
        ...
        # Si hay un pensamiento seleccionado, mostrar su gráfico de frecuencia
...
# Prior code remains the same until crear_grafico_frecuencia method, which changes to:
    ...
    def crear_grafico_frecuencia(self):
        ...
        fig = Figure(figsize=(6, 3))  # Altura reducida para dejar espacio a la descripción
        ax = fig.add_subplot(111)
        ...
        # Obtener y mostrar la descripción del pensamiento
        try:
            conn = sqlite3.connect('../../data/db_psicologia_clinic.db')
            cursor = conn.cursor()
